/*
    ...
    _|   \|  _|_ |   \ \__)  |   |  \___/  |   \
    	...
    	CodeChef - Power Sums (POWSUMS)
    	
    	Category: Mathematics
    	
    	Keys: Newton's Sums, Cantor-Zassenhaus Algorithm
...
int main (int argc, char const *argv[]) {
  ...
  for (int z = 1; z <= t; ++z) {
    ...
    // Algebraic manipulations for first subtask
    if (t > 1) {
      if (n == 3) {
    ...
    // Extraction of polynomial from Newton's identities 
    for (int i = n - 1; i >= 0; --i) {
      f.coeff[i] = 0;
